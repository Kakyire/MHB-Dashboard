...
3. The staff member enters reviewed plain text and source metadata.
4. `ingestWesleySource` stores the original text privately, creates a draft,
   chunks it, and generates embeddings.
5. The staff member explicitly calls `publishWesleySourceVersion` after review.

The public MHB application cannot use either admin operation. PDF upload and
OCR are deliberately a later phase; the first version accepts reviewed text so
that poor PDF extraction can never become a trusted source without inspection.
...
- Do not publish a draft until a Methodist editor has checked factual accuracy,
  source authority, and the right to use the text.
